...
char *String::fraseReversa()
{
 char *result = (char *)(malloc(sizeof(char) * strlen(this->info)));
 int lastLetterWord = strlen(this->info) - 1;
 bool lastLetterSetted = false;

 for (int i = strlen(this->info) - 1; i >= 0; i--)
 {
  if (!lastLetterSetted && ((int(this->info[i]) >= 61 && int(this->info[i]) <= 90) ||
                            (int(this->info[i]) >= 97 && int(this->info[i]) <= 122)))
  {
   lastLetterWord = i;
   lastLetterSetted = true;
  }
  else if (lastLetterSetted && (int(this->info[i]) < 61 || (int(this->info[i]) > 90 && int(this->info[i]) < 97) || int(this->info[i]) > 122) || (0 == i))
  {
   lastLetterSetted = false;

   char *word = new char[lastLetterWord - i];

   int tam = lastLetterWord - i;
   char *startIndex = this->info + i;
   if (i != 0)
   {
    startIndex++;
   }
